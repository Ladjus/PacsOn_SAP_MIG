...
LP_1 AS (
    ...
    FROM (
        SELECT
            LP.*,
            ROW_NUMBER() OVER (
                PARTITION BY LP.ForetagKod, LP.OrdLevPlats1
                ORDER BY LP.FtgNr
            ) AS rn
        FROM LP
    ) x
...
SELECT
    ...
    CASE
        ...
        WHEN LP.FtgNr IS NOT NULL THEN 'SHPT'
        ELSE ''
    END AS KTOKD,
    ...
    COALESCE(NULLIF(FR.ftgpostadr2, ''), '') AS FR_FTGPOSTADR2,
    ...
    CASE 
        WHEN LEN(FR.ean_loc_code) = 13 THEN LEFT(FR.ean_loc_code, 7) 
        ELSE '' 
    END AS LOCATION_1,

    CASE 
        WHEN LEN(FR.ean_loc_code) = 13 THEN SUBSTRING(FR.ean_loc_code, 8, 5) 
        ELSE '' 
    END AS LOCATION_2,

    CASE 
        WHEN LEN(FR.ean_loc_code) = 13 THEN RIGHT(FR.ean_loc_code, 1) 
        ELSE '' 
    END AS LOCATION_3,
    ...
    CASE 
        WHEN NULLIF(FR.webadress, '') IS NOT NULL THEN 'HPG' 
        ELSE '' 
    END AS URI_TYP,
    ...
    COALESCE(CAST(KUS.FtgNr AS varchar(50)), '') AS KUS_FTGNR,
    ...
    COALESCE(NULLIF(KUS.valkod, ''), '') AS KUS_VALKOD,
    ...
    COALESCE(NULLIF(LP.levsattkod, ''), '') AS LP_LEVSATTKOD,
    COALESCE(NULLIF(LP.q_ic_forwarderid, ''), '') AS LP_Q_IC_FORWARDERID,
    COALESCE(NULLIF(KUS.levsattkod, ''), '') AS KUS_LEVSATTKOD,
    COALESCE(NULLIF(KUS.q_ic_forwarderid, ''), '') AS KUS_Q_IC_FORWARDERID,
    ...
    COALESCE(NULLIF(LP.q_jis_phonenotifiedno, ''), '') AS LP_Q_JIS_PHONENOTIFIEDNO
